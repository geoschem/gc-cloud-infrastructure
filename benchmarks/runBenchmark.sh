...
then
    ...
    wget -r -nH --cut-dirs=3 -np -A "*.csv" geoschemdata.wustl.edu/ExtData/DataCatalogs/13.2.1/
...
ln -s build/bin/gchp gchp

# copy over local run script
cp /gchp.cloud.run gchp.cloud.run
...
aws s3 cp gchp.log "${S3_RUNDIR_PATH}outputDir/gchp.log"
aws s3 cp HEMCO.log "${S3_RUNDIR_PATH}outputDir/HEMCO.log"
aws s3 cp outputDir/ "${S3_RUNDIR_PATH}outputDir" --recursive
